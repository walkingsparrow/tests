    ...
    for (k in seq_len(max.iter)) {
        ...
        while (!stop1) {
            ...
            if (sqrt(sum(delta^2)) <= e2 * (sqrt(sum(phi^2) + sum(theta^2)))) {
                ...
                z1 <- get.errors(ts, p, q, phi.new, theta.new, z0)
                rho <- (sum(z^2) - sum(z1^2)) / (t(delta) %*% (mu*delta + g))
                if (rho > 0) {
                    phi <- phi.new
                    theta <- theta.new
                    ## z0 <- z
                    ## z <- get.errors(ts, p, q, phi, theta, z0)
                    z <- z1
...
r <- madlib.arima(ts - mean(ts), 0, 0, 1, include.mean = FALSE, max.iter = 400, tau = 1e-4, e1 = 1e-6, e2 = 1e-12, e3= 1e-8)
...
z <- get.errors(ts - 3.1971, 2, 1, c(0.7028, -0.3006), 0.1977)

sd(z)

z1 <- get.errors(ts - 3.0739, 2, 1, c(0.66421, -0.25679), 0.19791)
