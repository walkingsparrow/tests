        ...
        while (!stop) {
            ...
            if (sqrt(sum(delta^2)) <= e2 * (sqrt(sum(phi^2) + sum(theta^2)))) {
                ...
                if (rho > 0) {
                    ...
                    A <- t(jacob) %*% jacob
                    g <- t(jacob) %*% z
                    if (sqrt(mean(g^2)) <= e1 || mean(phi^2)+mean(theta^2) <= e3) stop <- TRUE
                    v <- 2
...
r <- madlib.arima(ts, 2,1,1, include.mean=FALSE, max.iter = 1000, tau = 1e-3, e1 = 1e-8, e2 = 1e-4, e3= 1e-8)

s <- arima(ts, order=c(2,1,1), include.mean=F, method="CSS")
s
